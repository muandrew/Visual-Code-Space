...
data class InternalDocumentFile(
  ...
  override val parentFile: File?
    get() = raw.parentFile?.let { InternalDocumentFile(it) }
  ...
  override fun createNewFile(fileName: String): File?
    = raw.createFile("", fileName)?.let { InternalDocumentFile(it) }

  override fun createNewDirectory(fileName: String): File?
    = raw.createDirectory(fileName)?.let { InternalDocumentFile(it) }
  ...
  override fun listFiles(): Array<out File>
    = raw.listFiles().map { InternalDocumentFile(it) }.toTypedArray()
