...
internal data class InternalJavaFile(
  ...
  override val parentFile: File?
    get() = raw.parentFile?.let { InternalJavaFile(it) }
  ...
  override fun createNewFile(fileName: String): File? {
    ...
    try {
      ...
      return InternalJavaFile(target)
    } catch (e: IOException) {
      return null
    }
  }
  ...
  override fun listFiles(): Array<out File>?
    = raw.listFiles()?.map { InternalJavaFile(it) }?.toTypedArray()

  override fun renameTo(newName: String): File? {
    val dest = java.io.File(raw.parentFile, newName)
    return if (raw.renameTo(dest)) InternalJavaFile(dest) else null
  }
...
fun java.io.File.toFile(): File = InternalJavaFile(this)